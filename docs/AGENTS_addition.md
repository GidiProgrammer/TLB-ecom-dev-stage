...
- **Never call `decrement_stock`, `restock_product`, `create_order_with_items`, or `create_quote_with_items` from client code.** These are `SECURITY DEFINER` functions with execution revoked from `anon`/`authenticated` on purpose. If a client-side call to one of these fails with a permissions/does-not-exist error, the fix is a server function using `supabaseAdmin` (service-role client from `src/integrations/supabase/client.server.ts`) — not loosening the grant.
- **Never add a client-writable RLS policy for `orders.status`, `orders.total`, or anything in `user_roles`.** If a feature seems to need the client to change one of these, it needs a server-side route instead. Admin/staff page access is checked with `requireStaffAccess` in `src/lib/staff.ts` (reads `user_roles` via `supabaseAdmin`), not by querying `user_roles` from the browser.
